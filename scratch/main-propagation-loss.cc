...
static Gnuplot
TestDeterministic (Ptr<FriisPropagationLossModel> model)
{
  ...
  model->SetLambda (3.0e8/5.0e9);
  Ptr<PropagationLossModel> modelGeneric = model;
...

  {
    ...
    for (double distance = 0.0; distance < 3000.0; distance += 10.0)
      {
        b->SetPosition (Vector (distance, 0.0, 0.0));

        // CalcRxPower() returns dBm.
        double rxPowerDbm = modelGeneric->CalcRxPower (txPowerDbm, a, b);
  ...
  plot.AddDataset ( Gnuplot2dFunction ("-101 dBm NoiseFloor", "-101.0") );
...
TestProbabilistic (Ptr<PropagationLossModel> model, unsigned int samples = 100000)
{
  ...
  // density for discrete distances.
  {
    ...
    for (double distance = 100.0; distance < 3000.0; distance += 100.0)
      {
        b->SetPosition (Vector (distance, 0.0, 0.0));
...
{
  GnuplotCollection gnuplots ("main-propagation-loss.png");
