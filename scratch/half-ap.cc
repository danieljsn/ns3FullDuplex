...
void experiment (bool enableCtsRts, uint32_t d1, uint32_t d2)
{
  ...
  wifiPhy.Set ("EnableCaptureEffect", BooleanValue (true));

  FullNqosWifiMacHelper wifiMac = FullNqosWifiMacHelper::Default ();
  wifiMac.Set ("EnableReturnPacket", BooleanValue (true));  //A -> B e B -> A
  wifiMac.Set ("EnableBusyTone", BooleanValue (false)); //A -> B e B -> BusyTone
  wifiMac.Set ("EnableForward", BooleanValue (true)); //A -> B e B -> C

  FullSsid ssid = FullSsid ("half-wifi-default");

  wifiMac.SetType ("ns3::FullStaWifiMac",
                   "Ssid", FullSsidValue (ssid),
                   "ActiveProbing", BooleanValue (false));
  wifiPhy.Set ("EnableFullDuplex", BooleanValue (false));
  NetDeviceContainer staDevice = wifi.Install (wifiPhy, wifiMac, nodes.Get(0));
  NetDeviceContainer devices = staDevice;
  wifiMac.SetType ("ns3::FullApWifiMac",
                   "Ssid", FullSsidValue (ssid));
  wifiPhy.Set ("EnableFullDuplex", BooleanValue (true));
  ...
  //wifiPhy.EnablePcap (enableCtsRts ? "full-rtscts-pcap-node2" : "full-ap-forward-test", nodes);
